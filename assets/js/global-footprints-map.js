(function () {
  ...
  function visiblePlaces() {
    return places.filter(function (place) { return matchesFilter(place, activeFilter); });
  }
  ...
  function renderMarkers() {
    ...
    visiblePlaces().forEach(function (place) {
      ...
      marker.on('click', function () { selectPlace(place, false); });
  ...
  function initMap() {
    ...
    markerLayer = window.L.markerClusterGroup ? window.L.markerClusterGroup({
      ...
      iconCreateFunction: function (cluster) {
        return window.L.divIcon({ html: '<div class="global-footprint-cluster">' + cluster.getChildCount() + '</div>', className: '', iconSize: [42, 42] });
      }
    }) : window.L.layerGroup();
    ...
    var first = visiblePlaces().filter(coordinatePair)[0];
    if (first) renderDetails(first);
    setTimeout(function () { map.invalidateSize(); fitVisibleMarkers(); }, 120);
  }

  function showError(message) {
    if (mapEl) mapEl.innerHTML = '<div class="home-global-map-error">' + escapeHtml(message) + '</div>';
  }
